...
task main()
{
	...
	{
		wait1Msec(30);
		float difference = nMotorEncoder[tubeLift]+ nMotorEncoder[driveL];
		float derivative = difference - lasterror;
		float turn = (Kp * difference)+(Kd*derivative);
		motor[driveR] = Tp + turn;
		motor[driveL] = Tp - turn;
		float error = lasterror;
		writeDebugStreamLine("L: %d, R: %d", nMotorEncoder[driveL], nMotorEncoder[tubeLift])
	}
}
